...
/*   Updated: 2023/12/14 04:12:47 by elias            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd > 4095 || BUFFER_SIZE <= 0)
		return (NULL);
	line = NULL;
	if (!first_read(&b_read, fd, &buff[fd], &i))
		return (NULL);
	while (b_read != B_READED_NULL || buff[fd])
	{
		if (get_line(&line, &buff[fd], &aux) == NULL)
			return (NULL);
		else if (aux == 1)
			return (line);
		i = 0;
		read_buff(&b_read, fd, &buff[fd], &line);
		if (b_read == B_READED_FAIL)
			return (NULL);
